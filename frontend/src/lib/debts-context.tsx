...
function loadDebts(): DebtItem[] {
  try {
    ...
    return Array.isArray(parsed)
      ? (parsed as DebtItem[]).map((d) => ({ ventas: [], ...d }))
      : [];
  } catch {
    return [];
  }
...
type DebtsContextValue = {
  ...
  addAbono: (clientId: number, monto: number, nota?: string) => void;
        ...
        debts.map((d) =>
          d.id !== clientId
            ...
            : {
                ...
                abonos: [
                  ...d.abonos,
                  { id: Date.now(), fecha: new Date().toISOString(), monto, nota },
                ],
              }
        )
      );
