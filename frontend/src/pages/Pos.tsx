...
function moneyBs(value: number, rate = 1): string {
...
function parseAmount(value: string): number {
  ...
  return parsed > 0 && Number.isFinite(parsed) ? parsed : 0;
}

function isSameDay(isoDate: string, dayKey: string): boolean {
  return new Date(isoDate).toISOString().slice(0, 10) === dayKey;
}

function formatPriceBsFirst(priceBs: number, rate: number) {
...
function formatCartTotal(totalBs: number, rate: number) {
...
function ModalCierreCaja({
  ...
  rate: number;
  cajero: string;
  salesSummary: CierreSalesSummary;
  hasPendingCart: boolean;
  onConfirm: (data: CierreData) => void;
  onCancel: () => void;
}) {
  const [efectivoDivisas, setEfectivoDivisas] = useState('');
  const [efectivoBolivares, setEfectivoBolivares] = useState('');
  const [pagoMovil, setPagoMovil] = useState('');
  const [puntoVenta, setPuntoVenta] = useState('');
  const [creditosPagadosManual, setCreditosPagadosManual] = useState(String(salesSummary.creditosPagados.toFixed(2)));
  const [divisasRestante, setDivisasRestante] = useState('');
  const [bolivaresRestante, setBolivaresRestante] = useState('');
  const [responsable, setResponsable] = useState('');
  const [montoDistribucion, setMontoDistribucion] = useState('');
  const [distribucion, setDistribucion] = useState<Array<{ responsable: string; monto: number }>>([]);
  const [cierresParciales, setCierresParciales] = useState<CierreParcial[]>([]);
  const [nombreParcial, setNombreParcial] = useState('Cierre 1');
  const [validationError, setValidationError] = useState('');

  const efectivoDivisasNum = parseAmount(efectivoDivisas);
  const efectivoBolivaresNum = parseAmount(efectivoBolivares);
  const pagoMovilNum = parseAmount(pagoMovil);
  const puntoVentaNum = parseAmount(puntoVenta);
  const creditosPagadosNum = parseAmount(creditosPagadosManual);
  const divisasRestanteNum = parseAmount(divisasRestante);
  const bolivaresRestanteNum = parseAmount(bolivaresRestante);

  const efectivoDivisasBs = round2(efectivoDivisasNum * rate);
  const totalCajaActual = round2(
    efectivoDivisasBs + efectivoBolivaresNum + puntoVentaNum + pagoMovilNum + creditosPagadosNum,
  );
  const totalCajaParciales = round2(cierresParciales.reduce((acc, parcial) => acc + parcial.totalCaja, 0));
  const totalCaja = round2(totalCajaParciales + totalCajaActual);
  const totalVenta = round2(salesSummary.totalVenta);
  const diferencia = round2(totalCaja - totalVenta);
  const estadoCuadre: CierreData['estadoCuadre'] = diferencia === 0 ? 'cuadrado' : diferencia > 0 ? 'sobrante' : 'faltante';
  const totalDejadoCaja = round2((divisasRestanteNum * rate) + bolivaresRestanteNum);
  const totalDistribuido = round2(distribucion.reduce((acc, item) => acc + item.monto, 0));
  const disponibleDistribucion = round2(totalCaja - totalDejadoCaja);
  const distribucionValida = totalDistribuido <= disponibleDistribucion;

  function addPartialClose() {
    const parcialTotal = totalCajaActual;
    const parcial: CierreParcial = {
      id: crypto.randomUUID(),
      nombre: nombreParcial.trim() || `Cierre ${cierresParciales.length + 1}`,
      efectivoDivisas: efectivoDivisasNum,
      efectivoBolivares: efectivoBolivaresNum,
      pagoMovil: pagoMovilNum,
      puntoVenta: puntoVentaNum,
      creditosPagados: creditosPagadosNum,
      totalCaja: parcialTotal,
    };
    if (parcialTotal <= 0) {
      setValidationError('El cierre parcial debe tener al menos un monto mayor a 0.');
      return;
    }
    setCierresParciales((prev) => [...prev, parcial]);
    setNombreParcial(`Cierre ${cierresParciales.length + 2}`);
    setEfectivoDivisas('');
    setEfectivoBolivares('');
    setPagoMovil('');
    setPuntoVenta('');
    setCreditosPagadosManual('0.00');
    setValidationError('');
  }

  function addDistribution() {
    const monto = parseAmount(montoDistribucion);
    if (!responsable.trim() || monto <= 0) {
      setValidationError('Ingresa responsable y monto de distribución válido.');
      return;
    }
    setDistribucion((prev) => [...prev, { responsable: responsable.trim(), monto: round2(monto) }]);
    setResponsable('');
    setMontoDistribucion('');
    setValidationError('');
  }

  function handleConfirm() {
    if (hasPendingCart) {
      setValidationError('Debes facturar o limpiar la venta pendiente antes de cerrar caja.');
      return;
    }
    if (totalDejadoCaja > totalCaja) {
      setValidationError('El dinero dejado en caja no puede ser mayor al total en caja.');
      return;
    }
    if (!distribucionValida) {
      setValidationError('La suma distribuida excede el monto disponible (total caja - dejado en caja).');
      return;
    }
    const data: CierreData = {
      cajero,
      subtotalProductos: round2(salesSummary.subtotalProductos),
      cantidadProductos: salesSummary.cantidadProductos,
      creditosOtorgados: round2(salesSummary.creditosOtorgados),
      totalVenta,
      efectivoDivisas: round2(efectivoDivisasNum),
      efectivoDivisasBs,
      efectivoBolivares: round2(efectivoBolivaresNum),
      puntoVenta: round2(puntoVentaNum),
      pagoMovil: round2(pagoMovilNum),
      creditosPagados: round2(creditosPagadosNum),
      totalCaja,
      diferencia,
      estadoCuadre,
      cierresParciales: cierresParciales.map((parcial) => ({
        nombre: parcial.nombre,
        efectivoDivisas: parcial.efectivoDivisas,
        efectivoBolivares: parcial.efectivoBolivares,
        puntoVenta: parcial.puntoVenta,
        pagoMovil: parcial.pagoMovil,
        creditosPagados: parcial.creditosPagados,
        totalCaja: parcial.totalCaja,
      })),
      divisasRestante: round2(divisasRestanteNum),
      bolivaresRestante: round2(bolivaresRestanteNum),
      totalDejadoCaja,
      distribucion: distribucion.map((item) => ({ ...item })),
    };
    onConfirm(data);
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="w-full max-w-5xl rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 my-8">
        {/* Header */}
        ...
        <div className="p-5 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 dark:border-primary/20">
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide text-primary">Resumen automático de ventas</h4>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <div>
                <p className="text-xs text-slate-500">Subtotal productos</p>
                <p className="font-extrabold text-slate-900 dark:text-white">{salesSummary.subtotalProductos.toFixed(2)} Bs</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Cantidad productos</p>
                <p className="font-extrabold text-slate-900 dark:text-white">{salesSummary.cantidadProductos}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Créditos otorgados</p>
                <p className="font-extrabold text-slate-900 dark:text-white">{salesSummary.creditosOtorgados.toFixed(2)} Bs</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Total venta</p>
                <p className="font-extrabold text-slate-900 dark:text-white">{totalVenta.toFixed(2)} Bs</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400">Cuadre de caja (entradas manuales)</h4>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Efectivo en divisas (USD)</label>
                <input type="number" value={efectivoDivisas} onChange={(e) => setEfectivoDivisas(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Efectivo en bolívares</label>
                <input type="number" value={efectivoBolivares} onChange={(e) => setEfectivoBolivares(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Punto de venta</label>
                <input type="number" value={puntoVenta} onChange={(e) => setPuntoVenta(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Pago móvil</label>
                <input type="number" value={pagoMovil} onChange={(e) => setPagoMovil(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Créditos pagados</label>
                <input type="number" value={creditosPagadosManual} onChange={(e) => setCreditosPagadosManual(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
            </div>

            <div className="mt-3 grid grid-cols-2 gap-3 rounded-lg border border-dashed border-slate-300 p-3 dark:border-slate-700 md:grid-cols-4">
              <div>
                <p className="text-xs text-slate-500">Divisas en Bs (automático)</p>
                <p className="text-sm font-bold text-slate-900 dark:text-white">{efectivoDivisasBs.toFixed(2)} Bs</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Total caja actual</p>
                <p className="text-sm font-bold text-slate-900 dark:text-white">{totalCajaActual.toFixed(2)} Bs</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Parciales acumulados</p>
                <p className="text-sm font-bold text-slate-900 dark:text-white">{totalCajaParciales.toFixed(2)} Bs</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Total en caja</p>
                <p className="text-sm font-bold text-slate-900 dark:text-white">{totalCaja.toFixed(2)} Bs</p>
              </div>
            </div>
          </div>

          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400">Cierres parciales (opcional)</h4>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Nombre parcial</label>
                <input type="text" value={nombreParcial} onChange={(e) => setNombreParcial(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
              <div>
                <button onClick={addPartialClose} type="button" className="mt-5 w-full rounded-xl border border-primary py-2 text-sm font-semibold text-primary hover:bg-primary/5">+ Registrar cierre parcial</button>
              </div>
            </div>
            {cierresParciales.length > 0 && (
              <div className="mt-3 space-y-2">
                {cierresParciales.map((parcial) => (
                  <div key={parcial.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm dark:bg-slate-800">
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{parcial.nombre}</span>
                    <span className="font-bold text-slate-900 dark:text-white">{parcial.totalCaja.toFixed(2)} Bs</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400">Dinero dejado en caja</h4>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Divisas restante (USD)</label>
                <input type="number" value={divisasRestante} onChange={(e) => setDivisasRestante(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Bolívares restante</label>
                <input type="number" value={bolivaresRestante} onChange={(e) => setBolivaresRestante(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              </div>
            </div>
            <p className="mt-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Total dejado en caja: {totalDejadoCaja.toFixed(2)} Bs</p>
          </div>

          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400">Distribución de efectivo</h4>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
              <input type="text" value={responsable} onChange={(e) => setResponsable(e.target.value)} placeholder="Responsable" className="rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              <input type="number" value={montoDistribucion} onChange={(e) => setMontoDistribucion(e.target.value)} placeholder="Monto Bs" className="rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              <button onClick={addDistribution} type="button" className="rounded-xl border border-primary py-2 text-sm font-semibold text-primary hover:bg-primary/5">+ Agregar</button>
            </div>
            {distribucion.length > 0 && (
              <div className="mt-3 space-y-2">
                {distribucion.map((item, index) => (
                  <div key={`${item.responsable}-${index}`} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm dark:bg-slate-800">
                    <span>{item.responsable}</span>
                    <span className="font-bold">{item.monto.toFixed(2)} Bs</span>
                  </div>
                ))}
              </div>
            )}
            <p className={`mt-2 text-xs font-semibold ${distribucionValida ? 'text-slate-500' : 'text-danger'}`}>
              Distribuido: {totalDistribuido.toFixed(2)} Bs / Disponible: {disponibleDistribucion.toFixed(2)} Bs
            </p>
          </div>

          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            <h4 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400">Validación de cuadre</h4>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <p className="font-extrabold text-slate-900 dark:text-white">{totalCaja.toFixed(2)} Bs</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Diferencia</p>
                <p className={`font-extrabold ${diferencia === 0 ? 'text-success' : diferencia > 0 ? 'text-warning' : 'text-danger'}`}>
                  {diferencia > 0 ? '+' : ''}{diferencia.toFixed(2)} Bs
                </p>
              </div>
            </div>
            <p className="mt-2 text-sm font-semibold">
              Estado: {estadoCuadre === 'cuadrado' ? 'Caja cuadrada' : estadoCuadre === 'sobrante' ? 'Sobrante' : 'Faltante'}
            </p>
          </div>

          {hasPendingCart && (
            <div className="rounded-xl border border-warning/30 bg-warning/5 p-4 dark:border-warning/20">
              <p className="text-sm font-bold text-warning">Hay una venta pendiente en el carrito.</p>
              <p className="text-xs text-slate-500 mt-1">Para mantener coherencia del cierre, completa o limpia el carrito antes de confirmar.</p>
            </div>
          )}
...
export function Pos() {
  ...
  const enabledPaymentMethods = useMemo(
    () => settings.metodos_habilitados,
    [settings.metodos_habilitados],
  );

  const cierreSalesSummary = useMemo<CierreSalesSummary>(() => {
    const todayKey = new Date().toISOString().slice(0, 10);
    const salesToday = sales.filter((sale) => isSameDay(sale.fecha, todayKey));
    const subtotalProductos = round2(salesToday.reduce((acc, sale) => acc + sale.total, 0));
    const cantidadProductos = salesToday.reduce(
      (acc, sale) => acc + sale.productos.reduce((sub, product) => sub + product.qty, 0),
      0,
    );
    const creditosOtorgados = round2(
      salesToday.filter((sale) => sale.metodo === 'fiado').reduce((acc, sale) => acc + sale.total, 0),
    );
    const creditosPagados = round2(
      debts.reduce(
        (acc, debt) =>
          acc + debt.abonos.reduce(
            (abonoAcc, abono) => abonoAcc + (isSameDay(abono.fecha, todayKey) ? abono.monto : 0),
            0,
          ),
        0,
      ),
    ...
    return {
      ...
      cantidadProductos,
      creditosOtorgados,
      totalVenta: round2(subtotalProductos - creditosOtorgados),
      creditosPagados,
    };
  }, [debts, sales]);
  ...
  function handleCheckout() {
    ...
    if (!enabledPaymentMethods[paymentMethod]) {
      setError('Este método de pago está deshabilitado en configuración.');
      return;
    }
  ...
  async function submitFiado(clientId: number) {
    ...
    try {
      await fetch(`${API_BASE}/sales`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sale_id: record.id, cajero: cashierName, productos: cart.map((i) => ({ id: i.id, qty: i.qty })), metodo: 'fiado', total, cliente_id: clientId }),
      });
    } catch { /* silencioso */ }
    setIsSubmitting(false);
    setLastSaleId(record.id);
  ...
  const visiblePaymentMethods = PAYMENT_METHODS.filter(({ key }) => enabledPaymentMethods[key]);

  useEffect(() => {
    if (paymentMethod && !enabledPaymentMethods[paymentMethod]) {
      setPaymentMethod(null);
    }
            ...
            <Card.Content className="space-y-4 p-4">
              ...
              <div className="space-y-2 border-t border-slate-200 pt-3 dark:border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {visiblePaymentMethods.map(({ key, label, icon }) => (
                    <button
                      key={key}
                      type="button"
        ...
        <ModalCierreCaja
          ...
          onConfirm={(data) => {
            ...
            addShift({
              ...
              efectivoDivisas: data.efectivoDivisas,
              efectivoDivisasBs: data.efectivoDivisasBs,
              efectivoBolivares: data.efectivoBolivares,
              puntoVenta: data.puntoVenta,
              pagoMovil: data.pagoMovil,
              creditosPagados: data.creditosPagados,
              totalCaja: data.totalCaja,
              diferencia: data.diferencia,
              estadoCuadre: data.estadoCuadre,
              cierresParciales: data.cierresParciales,
              divisasRestante: data.divisasRestante,
              bolivaresRestante: data.bolivaresRestante,
              totalDejadoCaja: data.totalDejadoCaja,
