...
function moneyUsd(v: number, rate: number) { return `$${(rate > 0 ? v / rate : 0).toFixed(2)} USD`; }
...
export function Debts() {
  ...
  const getRateByDate = (isoDate?: string) => {
    if (!isoDate) return rate;
    return getRateForDate(history, isoDate, rate);
  };
  ...
  function handleSaveAbono() {
    ...
    setIsSaving(true);
    addAbono(selected.id, monto, abonoForm.nota || undefined, rate);
    setIsSaving(false);
    setShowAbono(false);
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      {debts.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            { label: 'Deuda total acumulada', main: moneyUsd(totalDeuda, rate), sub: moneyBs(totalDeuda), color: 'text-slate-900 dark:text-white' },
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className={`text-sm font-bold ${isOver ? 'text-red-600' : 'text-slate-800 dark:text-slate-200'}`}>
                            {moneyUsd(d.deuda_total, rate)}
                          </p>
                          <p className="text-xs text-slate-400">{moneyBs(d.deuda_total)}</p>
                        </div>
                      </div>
                      <p className="mt-0.5 text-[11px] text-slate-400">Límite: {moneyUsd(d.limite_credito, rate)} ({moneyBs(d.limite_credito)})</p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/50">
                      ...
                      <p className={`mt-0.5 text-xl font-extrabold ${selected.deuda_total > selected.limite_credito ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}>
                        {moneyUsd(selected.deuda_total, rate)}
                      </p>
                      <p className="text-xs text-slate-400">{moneyBs(selected.deuda_total)}</p>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/50">
                      <p className="text-xs font-semibold uppercase text-slate-500">Límite de crédito</p>
                      <p className="mt-0.5 text-xl font-extrabold text-slate-900 dark:text-white">{moneyUsd(selected.limite_credito, rate)}</p>
                      <p className="text-xs text-slate-400">
                        Disponible: {moneyUsd(Math.max(0, selected.limite_credito - selected.deuda_total), rate)} ({moneyBs(Math.max(0, selected.limite_credito - selected.deuda_total))})
                      </p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selected.ventas.map((vid) => {
                          ...
                          const saleRate = sale ? getRateByDate(sale.fecha) : rate;
                          return (
                            <div key={vid} className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                              <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 px-3 py-2">
                                ...
                                {sale && (
                                  <div className="text-right">
                                    <p className="text-xs font-bold text-slate-700 dark:text-slate-200">{moneyUsd(sale.total, sale.tasa ?? saleRate)}</p>
                                    <p className="text-[11px] text-slate-400">{moneyBs(sale.total)}</p>
                                  </div>
                                )}
                              </div>
                                ...
                                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                  {sale.productos.map((p, i) => (
                                    <div key={i} className="flex items-center justify-between px-3 py-1.5 text-xs">
                                      ...
                                      <div className="text-right">
                                        <p className="font-semibold text-slate-600 dark:text-slate-300">{moneyUsd(p.price * p.qty, sale?.tasa ?? saleRate)}</p>
                                        <p className="text-[11px] text-slate-400">{moneyBs(p.price * p.qty)}</p>
                                      </div>
                                    </div>
                                  ))}
                                </div>
                        ...
                        {[...selected.abonos].reverse().map((a) => (
                          a.monto > 0 && (
                            <div key={a.id} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-800">
                              <div>
                                <p className="text-sm font-semibold text-green-600">+ {moneyUsd(a.monto, a.tasa ?? getRateByDate(a.fecha))}</p>
                                <p className="text-xs text-slate-500">({moneyBs(a.monto)})</p>
      ...
      {showAbono && selected && (
        <Modal title={`Abono — ${selected.nombre}`} onClose={() => setShowAbono(false)}>
          <div className="space-y-3">
            <p className="text-sm text-slate-500">Deuda pendiente: <span className="font-bold text-red-600">{moneyUsd(selected.deuda_total, rate)} ({moneyBs(selected.deuda_total)})</span></p>
